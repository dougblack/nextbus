...
public class RoutePager extends Activity implements OnSharedPreferenceChangeListener {
		...
		nearestStopsButton.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View arg0, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					...
					Intent nearestStopsActivity = new Intent(getApplicationContext(), NearestStops.class);
	...
	private class RoutePagerAdapter extends PagerAdapter implements TitleProvider {
		...
		public Object instantiateItem(View container, int position) {
			if (activeRoutesExist) {
				...
				String[] stopIdsTemp = null;
				if (thisRouteHasDirection) {
					itemListTemp = data.getDirectionList(currentRoutes[position]);
				} else {
					Object[] stopsAndIds = data.getStopList(currentRoutes[position]);
					itemListTemp = (String[]) stopsAndIds[0];
					stopIdsTemp = (String[]) stopsAndIds[1];
				}

				final String[] itemList = itemListTemp;
				final String[] stopIds = stopIdsTemp;
				...
				stopList.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
						if (thisRouteHasDirection) {
							...
							intent.putExtra("stopid", stopIds[position]);
							intent.putExtra("route", currentRoutes[listPosition]);
